...
const Navbar = () => {
  ...
  return (
    <div className='flex w-screen h-[10vh] rounded-b-3xl shadow-xl p-4 pb-0 pr-8 justify-between sticky top-0 z-50 bg-white box-border'>
      <Link href='/' passHref><Image src='/logo.png' alt='Logo' width={200} height={90} className='cursor-pointer'/></Link>

      <div className='hidden md:flex space-x-20 items-center text-[var(--primary-color)] text-xl h-[100%] relative' ref={containerRef}>

        {links.map((link)=>(
          <div key={link.path} className='flex justify-center items-center h-[100%]' data-path={link.path}>
      ...
      <div className='md:hidden'>
        <button onClick={() => setIsOpen(!isOpen)} className='text-[var(--primary-color)]'>
          {isOpen ? (
      ...
      {isOpen && (
        <div className="absolute top-[10vh] left-0 w-full bg-white shadow-md flex flex-col items-center space-y-4 md:hidden z-10 px-4 py-4">
          {links.map((link)=>(
            <div key={link.path} className='flex justify-center items-center h-[100%] w-[100%]'>
